...
export async function sendWelcomeEmail({ to, name, businessName, tempPassword, loginUrl }: WelcomeEmailArgs) {
  const subject = `Welcome to Fotiqo — ${businessName} is live!`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #0EA5A5;">Welcome to Fotiqo!</h1>
      <p>Hi ${name},</p>
      ...
      <a href="${loginUrl}" style="display: inline-block; background: #0EA5A5; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">
        Log In Now
      </a>
