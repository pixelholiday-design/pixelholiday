...
import { BarChart3, Loader2, Mail, Lock, ArrowRight, ArrowLeft } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (res?.error) { setErr("Invalid credentials — check your email and password."); return; }
    router.push("/admin/dashboard");
  }

  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-cream-100">
      <aside className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden bg-gradient-to-br from-coral-600 via-coral-500 to-coral-400">
        ...
        <div className="relative z-10 text-white max-w-md">
          <h2 className="font-display text-4xl xl:text-5xl leading-tight mb-4">Operations control center</h2>
          <p className="text-white/80 text-lg leading-relaxed">
            Full dashboard access for managers, supervisors, and operations leads.
          </p>
        </div>
        <div />
      </aside>
      ...
      <main className="flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm animate-slide-up">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="heading text-3xl">Admin sign in</h1>
          </div>
          <p className="text-navy-400 mb-8">CEO, Operations Manager, Supervisor access</p>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input className="input pl-10" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@fotiqo.local" autoComplete="email" required />
              </div>
            </div>
            <div>
          ...
          <p className="mt-8 text-xs text-navy-400 text-center">Protected by Fotiqo · GDPR compliant</p>
        </div>
      </main>
    </div>
  );
